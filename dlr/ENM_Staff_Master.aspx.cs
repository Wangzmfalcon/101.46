...
public partial class Home : System.Web.UI.Page
{
    ...
    public void statuschk(DataTable dt)
    {
        ...
        for (int i = 0; i < dt.Rows.Count;i++ )
        {
            ...
            string SQL_query = " select * from ENM_Staff_Master_List "
                + " where Record_S ='1' and Staff_No='" + row["Staff_No"].ToString().Trim() + "'  and Name='" + row["Name"].ToString().Trim() + "'";
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.Conn, CommandType.Text, SQL_query))
            {
                if (rdr.Read())
                {
                    ...
                    { 
                      string SQL_UP = " UPDATE ENM_Staff_Master_List  "
    ...
    public bool InsertToDataBase(DataSet ds,DataTable dt)
    {
        ...
        //读取原纪录
        string SQL_query = " select * from ENM_Staff_Master_List where Record_S ='1'";
        ...
        //}
        //修改旧记录
        string SQL_Updateold = " UPDATE ENM_Staff_Master_List SET Record_S = '0'where Record_S='1' ";//0表示旧的
        using (SqlConnection conn = new SqlConnection(SqlHelper.Conn))
        {
        ...
        string SQL_Insert =//插入
           "INSERT INTO ENM_Staff_Master_List (Staff_No,Name,AU_EXP,Licence_No,AM_EXP,Job_Title) " +
           "VALUES(@sn,@sna,@aue,@ln,@ame,@jt)";

        SqlParameter[] parms = new SqlParameter[]{
            new SqlParameter("@sn", SqlDbType.VarChar, 10),
            new SqlParameter("@sna", SqlDbType.VarChar, 30),
            new SqlParameter("@aue", SqlDbType.Date),
            new SqlParameter("@ln", SqlDbType.VarChar, 20),
            new SqlParameter("@ame", SqlDbType.Date),
            new SqlParameter("@jt", SqlDbType.VarChar, 20)
         };
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "au")
        {
            ...
            string SQL_UP = " UPDATE ENM_Staff_Master_List  "
                + " SET AU_EXP_S  ='2'"
                + " where Record_S='1'  and Staff_No ='" + gvr.Cells[0].Text.ToString().Trim() + "' and Name ='" + gvr.Cells[1].Text.ToString().Trim() + "'";
            using (SqlConnection conn = new SqlConnection(SqlHelper.Conn))
            {
                SqlHelper.ExecuteNonQuery(conn, CommandType.Text, SQL_UP);
            }
