...
public partial class Home : System.Web.UI.Page
{
        ...
        for (int i = 0; i < dt.Rows.Count;i++ )
        {
            ...
            using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.Conn, CommandType.Text, SQL_query))
            {
                if (rdr.Read())
                {
                    ...
                    if (rdr["Insurance"].ToString().Trim() == row["Insurance"].ToString().Trim())//AU检测
                    {
                        string SQL_UP = " UPDATE DLR_Ceritifcate_Monitor  "
                        + " SET Insurance_S  ='" + row["Insurance_S"].ToString().Trim() + "',Insurance_P ='" + row["Insurance_P"].ToString().Trim() + "'"
                        + " where Registration = '" + row["Registration"].ToString().Trim() + "'";
                        using (SqlConnection conn = new SqlConnection(SqlHelper.Conn))
                        {
                            SqlHelper.ExecuteNonQuery(conn, CommandType.Text, SQL_UP);
    ...
    public bool InsertToDataBase(DataSet ds,DataTable dt)
    {
        ...
        dt.Columns.Add("Insurance_P", typeof(string));
        dt.Columns.Add("Insurance_S", typeof(string));
        ...
        using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.Conn, CommandType.Text, SQL_query))
        {
            ...
            while (rdr.Read())
            {
                ...
                row["Insurance"] = Convert.ToString(rdr.GetSqlValue(10));
                row["Insurance_P"] = Convert.ToString(rdr.GetSqlValue(11));
                row["Insurance_S"] = Convert.ToString(rdr.GetSqlValue(12));
                dt.Rows.Add(row);
            }
        }
        //删除
        ...
        string SQL_Insert =//插入
           "INSERT INTO DLR_Ceritifcate_Monitor (Registration,C_of_A,Station_Licence,Radio_Licence,Insurance) " +
         ...
         else
         {
             ...
             if (k == 0)
             {
                 ...
                 for (int j = GridView1.PageIndex * GridView1.PageSize, i = 0; i < GridView1.PageSize && j < dt.Rows.Count; i++, j++)
                 {
                     ...
                     //GridView1.Rows[i].Cells[13].Text = Convert.ToDateTime(dt.Rows[j]["Insurance"]).ToShortDateString();
                     //GridView1.Rows[i].Cells[14].Text = dt.Rows[j]["Insurance_P"].ToString();
                     //GridView1.Rows[i].Cells[15].Text = getstatus(dt.Rows[j]["Insurance_S"].ToString());
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "IN")
        {
            ...
            string SQL_UP = " UPDATE DLR_Ceritifcate_Monitor  "
                + " SET Insurance_S  ='2'"
                + " where Registration ='" + gvr.Cells[0].Text.ToString().Trim() +"' and Insurance ='" + gvr.Cells[5].Text.ToString().Trim() + "'";
            using (SqlConnection conn = new SqlConnection(SqlHelper.Conn))
            {
                SqlHelper.ExecuteNonQuery(conn, CommandType.Text, SQL_UP);
            }
