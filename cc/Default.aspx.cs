...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void login_Click(object sender, EventArgs e)
    {
        ...
        string SQL_Login =
            "select * from Crew_Check_User where Crew_Check_User.User_ID =@userid COLLATE  Chinese_PRC_CS_AS";
        SqlParameter[] parm = new SqlParameter[]{
                new SqlParameter("@userid", SqlDbType.VarChar, 50)
            };
        parm[0].Value = uid;
        using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.Conn, CommandType.Text, SQL_Login, parm))
        {
            if (rdr.Read())
            {
