...
public partial class crewcheck : System.Web.UI.Page
{
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if(sN!=""&& Ty!=""&&Ob!=""&&Fr!=""&&To!=""&&Ex!=""&&Re!="")
        {
            ...
            //查旧记录
                        ...
                        string keyword1 = Convert.ToDateTime(keyword).AddSeconds(1).ToString();
                        string SQL_query = " select * from Crew_Check"
                    + " where Crew_Check.Processed_At >='" + keyword + "'  and Crew_Check.Processed_At <= '"+keyword1+"'";
                ...
                using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.Conn, CommandType.Text, SQL_query))
                {
                    if (rdr.Read())
                    {
                        ...
                        parmlog[1].Value = rdr[0].ToString() + "/" + rdr[1].ToString() + "/" + rdr[2].ToString() + "/" + rdr[3].ToString() + "/" + rdr[4].ToString() + "/" + rdr[5].ToString();
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (((DropDownList)e.Row.FindControl("staffNo")) != null)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                staffNoDR.Items.Add(new ListItem(dt.Rows[i]["StaffNo(StaffName)"].ToString()));
            }
    ...
    protected void Bind() 
    {
        ...
        using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.Conn, CommandType.Text, SQL_Query, parm))
        {
            while (rdr.Read())
            {
                ...
                row["Processed_At"] = Convert.ToString(rdr.GetSqlValue(9));
                dt.Rows.Add(row);
            }

        }
        GridView1.DataSource = dt;//绑定数据显示   
        GridView1.DataKeyNames = new String[] { "Processed_At" };
        GridView1.DataBind();
   ...
   //插入新数据
    ...
    {
        Response.Redirect("InsertCC.aspx");
    }
    //删除
