...
public partial class crewcheck : System.Web.UI.Page
{
    ...
    { 
       ...
       string SQL_getstaff = " select Staff_No from Crew_Info "
       + "where Crew_Info.Rcd_Status='NEW'   AND Title='" + type + "'";


       SqlParameter staffno = new SqlParameter();

       using (SqlDataReader rdr = SqlHelper.ExecuteReader(SqlHelper.Conn, CommandType.Text, SQL_getstaff))
       {
           while (rdr.Read())
           {
               staffno.Value = rdr[0].ToString().Trim();
               stafflist.Add(staffno.Value.ToString());
    ...
    public void insertCC(string sN, string Ty, string Ob, string Fr, string To, string Ex, string Re)//插入
    { 
         ...
         if (sN != "" && Ty != "" && Ob != "" && Fr != "" && To != "")
            {
                if (sN == "00000000")//ALL CREW
                {
                    ...
                    {
                        insertCC(staff, Ty, Ob, Fr, To, Ex, Re);
                    }
                }
